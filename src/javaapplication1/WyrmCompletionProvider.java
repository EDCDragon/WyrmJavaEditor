 ...
 * @version 1.0
 */
public class WyrmCompletionProvider extends AbstractCompletionProvider {
     ...
     * by {@code JTextComponent.getToolTipText()}, and if the user wiggles the
    ...
    AutoCompleteDemo.WyrmJSONObject object;
     ...
     */
    public WyrmCompletionProvider(AutoCompleteDemo.WyrmJSONObject object) {
        init();
        this.object = object;
    }
     ...
     * @see BasicCompletion
     */
    public WyrmCompletionProvider(String[] words) {
        init();
    ...
    public List<Completion> getCompletions(JTextComponent comp) {
        ...
        if (alreadyEnteredText.startsWith("element.")) {
            List<Completion> ret = new ArrayList<>();

            String[] splitAlreadyEnteredText = alreadyEnteredText.split("\\.");

            JSONObject lastValidLevel = object.getJSONObject("element");
            String lastInvalidString = "";
            String replacementText = "element";

            for (int i = 1; i < splitAlreadyEnteredText.length; i++) {
                String levelString = splitAlreadyEnteredText[i];

                if (lastValidLevel.has(levelString)) {
                    replacementText += "." + levelString;
                    lastValidLevel = lastValidLevel.getJSONObject(levelString);
                } else {
                    lastInvalidString = levelString;
                    break;
                }
            }

            for (String s : lastValidLevel.keySet()) {
                if (s.startsWith(lastInvalidString)) {
                    ret.add(new BasicCompletion(this, replacementText + "." + s));
                }
            }
    ...
    protected List<Completion> getCompletionsImpl(JTextComponent comp) {
        return super.getCompletionsImpl(comp); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public void setAutoActivationRules(boolean letters, String others) {
        super.setAutoActivationRules(letters, others); //To change body of generated methods, choose Tools | Templates.
    }
    ...
    protected void init() {
        ...
        this.addCompletion(new TemplateCompletion(this, "if", "if",
                "if(${i})\n"
                + "{\n"
                + "\n"
                + "}"
        ...
        this.addCompletion(new TemplateCompletion(this, "for", "for",
                "for(int ${i}=0; i < ${i}; i++)\n"
                + "{\n"
                + "\n"
                + "}"
        ...
        this.addCompletion(new TemplateCompletion(this, "while", "while",
                "while(${i})\n"
                + "{\n"
                + "\n"
                + "}"
        ...
        this.addBasicCompletions("boolean", "byte", "short", "int", "long", "float", "long");
        this.addBasicCompletions("Boolean", "Byte", "Short", "Integer", "Long", "Float", "Long");

        this.addBasicCompletions("element");
        
        this.setAutoActivationRules(false, ".");
    }

    public void addBasicCompletion(String s) {
        this.addCompletion(new BasicCompletion(this, s));
    }

    public void addBasicCompletions(String... ss) {
        for (String s : ss) {
            this.addCompletion(new BasicCompletion(this, s));
        }
    }
     ...
     * in the XML, such as custom {@link FunctionCompletion}s. This may be
